...
if ( move_uploaded_file( $lokasi_file, $folder_tn ) ) {
 $result = mysqli_query( $mysqli, "UPDATE `pengumuman` SET `JUDUL_PENGUMUMAN`='$judul',`DESKRIPSI_PENGUMUMAN`='$deskripsi',`TGL_PENGUMUMAN`='$tglpengumuman',`GAMBAR`='$nama_file',`UPDATE_TIME`='$create_time' WHERE PENGUMUMAN_ID='$id'" );
...
if ( $result) {
    ...
    echo "
	  <script type='text/javascript'>
		setTimeout(function () { 	
			swal({
				title: 'Data Pengumuman Gagal Diubah',
				type: 'error',
				timer: 1500,
				showConfirmButton: true
			});		
...
$result = mysqli_query( $mysqli, "UPDATE `pengumuman` SET `JUDUL_PENGUMUMAN`='$judul',`DESKRIPSI_PENGUMUMAN`='$deskripsi',`TGL_PENGUMUMAN`='$tglpengumuman',`UPDATE_TIME`='$create_time' WHERE PENGUMUMAN_ID='$id'" );
