...
// $result_log = mysqli_query( $mysqli, "INSERT INTO tb_log_activity (`nama_user`, `deskripsi`, `status`) VALUES ('$nama_user','$deskripsi_log','tambah')" );
 ...
 $result = mysqli_query( $mysqli, "UPDATE `donasi` SET `DESKRIPSI_UMUM`='$deskripsiUmum',`DESKRIPSI1`='$deskripsi1',`DESKRIPSI2`='$deskripsi2',`DESKRIPSI3`='$deskripsi3',`DESKRIPSI4`='$deskripsi4',`JUMLAH1`='$jumlah1',`JUMLAH2`='$jumlah2',`JUMLAH3`='$jumlah3',`JUMLAH4`='$jumlah4',`UPDATE_TIME`='$create_time' WHERE DONASI_ID='$id'" );
...
if ( $result) {
    echo "
	  <script type='text/javascript'>
		setTimeout(function () { 	
			swal({
				title: 'Data Donasi Berhasil Diubah',
				type: 'success',
				timer: 1500,
    ...
    echo "
	  <script type='text/javascript'>
		setTimeout(function () { 	
			swal({
				title: 'Data Donasi Gagal Diubah',
				type: 'error',
				timer: 1500,
				showConfirmButton: true
			});		
