...
$deskripsi_log = "Menambahkan Dokumen Penanganan dengan ID Penanganan $id";
...
foreach ( $_FILES['file']['tmp_name'] as $key=>$tmp_name ) {
    ...
    $folder_bg = "../dokumen/dokumenRehab/$file_tmp_bg";
    ...
    if ( in_array( $ext, $extension_doc ) ) {
        ...
        $result = mysqli_query( $mysqli, "INSERT INTO `dokumen_penanganan` (`dokumen`,`id_penanganan`) VALUES ('$file_tmp_bg','$id')" );
    } else {
...
if ( $result ) {
    echo "
	  <script type='text/javascript'>
		setTimeout(function () { 	
			swal({
				title: 'Dokumen Penanganan Berhasil Ditambahkan',
				type: 'success',
				timer: 1500,
    ...
    echo "
	  <script type='text/javascript'>
		setTimeout(function () { 	
			swal({
				title: 'Dokumen Penanganan Gagal Ditamabahkan',
				type: 'error',
				timer: 1500,
				showConfirmButton: true
			});		
