...
$deskripsi_log = "Menambahkan Foto Kondisi dengan ID Kondisi $id";
...
foreach ( $_FILES['file']['tmp_name'] as $key=>$tmp_name ) {
    ...
    $folder_bg = "../dokumen/fotoKondisi/$file_tmp_bg";
    $ext = pathinfo( $file_tmp_bg, PATHINFO_EXTENSION );

    if ( in_array( $ext, $extension ) ) {
        move_uploaded_file( $file_tmp_bg1 = $_FILES['file']['tmp_name'][$key], $folder_bg );
        $result = mysqli_query( $mysqli, "INSERT INTO `gambar_kondisi` (`gbr_kon`,`id_kondisi`) VALUES ('$file_tmp_bg','$id')" );
    } else {
...
if ( $result ) {
    echo "
	  <script type='text/javascript'>
		setTimeout(function () { 	
			swal({
				title: 'Foto Kondisi Berhasil Ditambahkan',
				type: 'success',
				timer: 1500,
    ...
    echo "
	  <script type='text/javascript'>
		setTimeout(function () { 	
			swal({
				title: 'Foto Kondisi Gagal Ditambahkan',
				type: 'error',
				timer: 1500,
				showConfirmButton: true
			});		
